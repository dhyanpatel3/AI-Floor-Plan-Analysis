...
export const calculateMaterials = (
  ...
  const { totalWallLengthFt, totalAreaSqFt, wallThicknessFt } =
    analysis.summary;
  const wallHeightFt = settings.wallHeightFt;

  // Convert to Metric for calculation consistency with constants
  const totalAreaSqM = totalAreaSqFt / SQFT_PER_M2;
  const totalWallLengthM = totalWallLengthFt / 3.28084;
  const wallThicknessM = wallThicknessFt / 3.28084;
  const wallHeightM = wallHeightFt / 3.28084;

  // 1. Structure Volume (in Cft for volume based, or convert back to M3 if needed)
  // The existing constants (BRICKS_PER_M3=500) imply M3 calculations.
  // Let's use the Metric values derived above to preserve the logic.

  const grossWallVol = totalWallLengthM * wallHeightM * wallThicknessM;
  const openingVol =
    (analysis.elements.doors * 1.89 + analysis.elements.windows * 1.8) *
    wallThicknessM;
  ...
  const totalCementBags = Math.ceil(
    netBrickworkVol * 1.26 +
      totalConcreteVol * 8.0 +
      totalWallLengthM * wallHeightM * 2 * 0.015 * 0.15 * 28,
  ); // Includes plaster
  const totalSteelKg = Math.ceil(totalAreaSqFt * STEEL_KG_PER_SQFT);
  const totalSandCft = Math.ceil(
    netBrickworkVol * 6 +
      totalConcreteVol * 15 +
      totalWallLengthM * wallHeightM * 2 * 0.015 * 1.2 * 35,
  );
  ...
  return items.map((item) => {
    // If custom rate is 0 or undefined, fallback to default rate
    const customRate = currentRates[item.id!];
    const rate =
      customRate && customRate > 0 ? customRate : RATES[item.id!] ?? 0;
...
export const calculateRoomMaterials = (
  ...
  const perimeterFt = room.perimeterFt;
  const wallAreaSqFt = perimeterFt * settings.wallHeightFt;
  ...
  if (room.type === "Bathroom") {
    items.push({
      ...
      unit: "Sq. Ft",
      quantity: Math.ceil(areaSqFt * 1.1),
    });
    ...
    items.push({
      ...
      unit: "Sq. Ft",
      quantity: Math.ceil(perimeterFt * tileHeight),
    });
    // Plumbing
    items.push({
      id: "wc_ewc",
    ...
    items.push({
      id: "plumbing_point",
      category: "Services",
      name: "Plumbing Points (Inlet/Outlet)",
      unit: "Pts",
      quantity: 5,
    }); // WC, Basin, Shower(2), Geyser
    items.push({
      id: "door_toilet",
      category: "Finishing",
      name: "PVC/WPC Door",
    ...
    items.push({
      ...
      unit: "Sq. Ft",
      quantity: Math.ceil(counterLenFt * 2.5),
    }); // 2.5ft width
    ...
    items.push({
      ...
      unit: "Sq. Ft",
      quantity: Math.ceil(counterLenFt * 2),
    });
    items.push({
      id: "kitchen_sink",
      category: "Services",
    ...
    items.push({
      ...
      name: "Vitrified Floor Tiles",
      unit: "Sq. Ft",
      quantity: Math.ceil(areaSqFt * 1.05),
    });
    // Paint (Wall + Ceiling)
    ...
    items.push({
      ...
      unit: "Kg",
      quantity: Math.ceil(paintArea / 14),
    });
    items.push({
      id: "primer",
      category: "Finishing",
      name: "Primer",
      unit: "Liters",
      quantity: Math.ceil(paintArea / 140),
    });
    items.push({
      id: "paint_emulsion",
      category: "Finishing",
      name: "Emulsion Paint",
      unit: "Liters",
      quantity: Math.ceil(paintArea / 120),
    });
  ...
  let elecPoints = 0;
  if (room.type === "Bedroom")
    elecPoints = 8; // 2 Light, 1 Fan, 2 Plug, 1 AC, 1 TV, 1 Bedside
  else if (room.type === "Living")
    elecPoints = 12; // More lights, TV, WiFi
  else if (room.type === "Kitchen")
    elecPoints = 6; // Light, Exhaust, Mixer, Fridge, Oven, RO
  else if (room.type === "Bathroom")
    elecPoints = 3; // Light, Exhaust, Geyser
  else elecPoints = 4;
  ...
  if (elecPoints > 0) {
    items.push({
      id: "electrical_point",
      category: "Services",
      name: "Electrical Points (Wiring+Switch)",
      unit: "Pts",
      quantity: elecPoints,
    });
  }
